...
bool CachePopulateMetadataFromEntry(const cJSON *entry, const char *path,
  ...
  if (!entry || !path || !out_md) {
    return false;
  }

  memset(out_md, 0, sizeof(ImageMetadata));
  out_md->path = strdup(path);
  if (!out_md->path) {
    return false;
  }

  cJSON *node = cJSON_GetObjectItem((cJSON *)entry, "modificationDate");
  if (node) {
    out_md->modificationDate = node->valuedouble;
  }

  node = cJSON_GetObjectItem((cJSON *)entry, "fileSize");
  if (node) {
    out_md->fileSize = (long)node->valuedouble;
  }

  node = cJSON_GetObjectItem((cJSON *)entry, "dateTaken");
  if (node && cJSON_IsString(node))
    strncpy(out_md->dateTaken, node->valuestring, METADATA_MAX_STRING - 1);
  node = cJSON_GetObjectItem((cJSON *)entry, "width");
  if (node)
    out_md->width = (int)node->valuedouble;
  node = cJSON_GetObjectItem((cJSON *)entry, "height");
  if (node)
    out_md->height = (int)node->valuedouble;
  node = cJSON_GetObjectItem((cJSON *)entry, "cameraMake");
  if (node && cJSON_IsString(node))
    strncpy(out_md->cameraMake, node->valuestring, METADATA_MAX_STRING - 1);
  node = cJSON_GetObjectItem((cJSON *)entry, "cameraModel");
  if (node && cJSON_IsString(node))
    strncpy(out_md->cameraModel, node->valuestring, METADATA_MAX_STRING - 1);
  node = cJSON_GetObjectItem((cJSON *)entry, "orientation");
  if (node)
    out_md->orientation = (int)node->valuedouble;
  node = cJSON_GetObjectItem((cJSON *)entry, "gpsLatitude");
  if (node) {
    out_md->hasGps = true;
    out_md->gpsLatitude = node->valuedouble;
    cJSON *lon = cJSON_GetObjectItem((cJSON *)entry, "gpsLongitude");
    if (lon)
      out_md->gpsLongitude = lon->valuedouble;
  }
  node = cJSON_GetObjectItem((cJSON *)entry, "exactHashMd5");
  if (node && cJSON_IsString(node))
    strncpy(out_md->exactHashMd5, node->valuestring, 32);

  node = cJSON_GetObjectItem((cJSON *)entry, "allMetadataJson");
  if (node && (cJSON_IsObject(node) || cJSON_IsArray(node))) {
    out_md->allMetadataJson = cJSON_PrintUnformatted(node);
  }

  CachePopulateMlFields(entry, out_md);
  return true;
}
...
bool CacheGetValidEntry(const char *absolute_path, double current_mod_date,
  ...
  if (!CachePopulateMetadataFromEntry(entry, absolute_path, out_md)) {
    return false;
  }
...
bool CacheGetRawEntry(const char *key, ImageMetadata *out_md) {
  ...
  return CachePopulateMetadataFromEntry(entry, key, out_md);
}
...
char **CacheGetAllKeys(int *out_count) {
  ...
  char **keys = malloc((size_t)count * sizeof(char *));
  if (!keys) {
    *out_count = 0;
    return NULL;
  }
