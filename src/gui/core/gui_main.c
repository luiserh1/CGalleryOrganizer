  ...
  while (!exit_requested) {
    ...
    if (show_exit_dialog) {
      ExitDialogAction action =
          GuiDrawUnsavedExitDialog(GuiGetFont(), g_gui_font_size);
      ...
      RebuildDialogAction action = GuiDrawRebuildConfirmDialog(
          GuiGetFont(), g_gui_font_size, state.rebuild_confirm_reason);
