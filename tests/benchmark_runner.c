...
static bool AppendBenchmarkRecord(const char *history_path,
  ...
  cJSON_AddNumberToObject(root, "rssStartBytes",
                          (double)record->rssStartBytes);
...
static bool WriteLastSummary(const char *last_path, const BenchmarkRecord *records,
                             int count) {
  ...
  for (int i = 0; i < count; i++) {
    ...
    cJSON_AddBoolToObject(entry, "success", records[i].success != 0);
...
int main(int argc, char **argv) {
  ...
  BenchmarkRecord records[3];
  memset(records, 0, sizeof(records));

  int failures = 0;
  for (int i = 0; i < workload_count; i++) {
    BenchmarkRecord *rec = &records[i];
  ...
  if (!WriteLastSummary(last_path, records, workload_count)) {
